  ...
  function bindAuthForms() {
    $("#signupForm").on("submit", async function (event) {
      ...
      try {
        ...
        uiService.showInlineErrors(validationService.mapServerErrors(error), "signup");
        if (!Object.keys(validationService.mapServerErrors(error)).length) {
    ...
    $("#loginForm").on("submit", async function (event) {
      ...
      try {
        ...
        uiService.showInlineErrors({ username: error.message, password: error.message }, "login");
      }
    });
  }
